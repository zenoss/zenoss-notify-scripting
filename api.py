...
# Let's set a timeout value of 5 seconds
QUERY_TIMEOUT = 5
## Let's set the default timeout (in seconds) for all socket connections
...
class ZAPI():
    ...
    def __init__(self, debug=False, targetInstance=targetInstance, zenossUser='admin', zenossPassword='zenoss'):
        ...
        handler=urllib2.HTTPSHandler(context=ssl._create_unverified_context())
        self.urlOpener = urllib2.build_opener(urllib2.HTTPCookieProcessor(),handler)
    ...
    def _router_request(self, router, method, data=[]):
        ...
        return json.loads(self.urlOpener.open(req, reqData, QUERY_TIMEOUT).read())
    ...
    def get_events2(self, device=None, component=None, deviceClass='network', eventClass=None):
        data = dict(start=0, limit=100, dir='DESC', sort='severity')
        data['params'] = dict(severity=[5], eventState=[0],DeviceClass=deviceClass)
    ...
    def get_events(self, device=None, component=None, deviceClass='network', eventClass=None, severity=[5,4,3,2], eventState=[0,1], prodState=[1000]):
        data = dict(start=0, limit=100, dir='DESC', sort='severity')
    ...
    def get_events_uid(self, device=None, component=None, uid="/zport/dmd/Groups/Office", eventClass=None, severity=[5,4,3,2], eventState=[0,1], prodState=[1000]):
        data = dict(start=0, limit=100, dir='DESC', sort='severity', uid=uid)
        data['params'] = dict(severity=severity, eventState=eventState)
